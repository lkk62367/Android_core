...
//        this.rawImagePublisher = connectedNode.newPublisher(resolver.resolve("image/raw"), sensor_msgs.Image._TYPE);
	...
	public void myProcessFrame()
	{
		...
		while (true) {
			i++;
			if ((i%2)==0) {
				bbmp = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.face1);
			}else
				bbmp = BitmapFactory.decodeResource(mContext.getResources(), R.drawable.face2);
			int oldwidth = bbmp.getWidth();
			int oldheight = bbmp.getHeight();
			float scaleWidth = 96 / (float) oldwidth;
			float scaleHeight = 96 / (float) oldheight;
			Matrix matrix = new Matrix();
			matrix.postScale(scaleWidth, scaleHeight);
			Bitmap resizedbbmp = Bitmap.createBitmap(bbmp, 0, 0, oldwidth, oldheight, matrix, true);
			...
			resizedbbmp.compress(Bitmap.CompressFormat.PNG, MainActivity.imageCompressionQuality, baos);
			...
			cameraInfo.setWidth(resizedbbmp.getWidth());
			cameraInfo.setHeight(resizedbbmp.getHeight());
